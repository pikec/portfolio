...
?>
    ...
    <!-- Display information already in the table -->
...
function addMovie()
{
    ...
    $MStudio = $_POST['MovieStudio'];
     /*
    //Check if studio is in the database if not spot error and exit function
    if (empty($MStudio))
    {
        $emptyStudio = "You must enter the name of a studio";
        echo "<script type='text/javascript'>alert('$emptyStudio ');</script>>";
        return;
    }

    //Select names of studios in the database we need to match the user's input
    $studioList = $mysqli->query("SELECT name, studio_id FROM studios");
    $Srows = $studioList->num_rows;

    $match =  0; //flag to catch if studio id is present
    $tempID = 0;

    //Compare user input to studio names in the database
    while ($row=mysqli_fetch_row($studioList))
    {
      if(strcasecmp($MStudio,$row[0]) == 0)
       {
            $match = 1; //set the studio match flag
            $tempID = $row[1]; //store the studio ID of the matching studio
       }
    }


    //The user entered a studio that did not match
    if ($match == 0)
    {
        $NoStudio = "Sorry you must enter the name of a studio that is already in the database";
        echo "<script type='text/javascript'>alert('$NoStudio');</script>>";
        return; //exit function 
    }

    //Now add the movie to the database
    $studioList->close(); //close the query*/
...
function addChar()
{
    //Establish connection to DB
        ...
        /*else if (empty($CMTitle))
        {
            $emptyCMtitle= "You must enter a movie title";
            echo "<script type='text/javascript'>alert('$emptyStudio ');</script>>";
            return;
        }/*

        /*Find the matching movie and actor the character appears in
        $List = $mysqli->query("SELECT actor_id FROM actors WHERE fname = ? AND lname = ?");
        $List2 = $mysqli->query("SELECT film_id FROM films WHERE title = ?");
